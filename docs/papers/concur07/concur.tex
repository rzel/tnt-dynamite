...
\MPS{To whom correspondence should be addressed.}{I've changed your `thanks' footnote}

\MPS{Phased Mobile System.}{I've changed your paper title}

\begin{abstract}
\MPS{We present a novel combination of process synchronisation and mobility
 ...
 {We present \emph{TNT}, a novel calculus combining explicit clocks and run-to-completion semantics with mobility, and demonstrate its suitability for modelling complex hierarachical systems with changing topology.}
\end{abstract}
...
}{\textit{Deleted first two paragraphs}}
...
\emph{synchronisation} to model \emph{interaction}.%
}
Indeed, this is the defining concept of CCS\MPS{\ADDTEXT}{ }\cite{milner:ccs}.
\MPS{Calculi}{In contrast, calculi} that include \emph{localities}
\MPS{\ADDTEXT}{-- for example the \emph{ambient calculus} \cite{amb} --}
also allow the notion of componentisation to be explicitly realised by \MPS{dividing}{forming}
processes into a hierarchy.  \MPS{One proponent of this technique is the
ambient calculus\MPS{\ADDTEXT}{ }\cite{amb}, which focuses on these entities (known as
`ambients' in this context) and allows their position to change during
execution.}{The inclusion of locality naturally leads to \emph{mobility},
the ability of a system to change its topological structure unilaterally.
In consequence, the ambient calculus and its analogues are well positioned
to model distributed systems in whicb resources can migrate from one
location to another.}

What tends to be absent from these models\MPS{\ADDTEXT}{, however,} is the notion of behaviour
also being governed by time, or at least, some notion of phasing.  Some
calculi maintain real time values\MPS{\ADDTEXT}{ }\cite{tccs,beaten:timing,brics:lee},
...
process algebras, such as TPL\MPS{\ADDTEXT}{ }\cite{hennessy:tpl}, PMC\MPS{\ADDTEXT}{ }\cite{pmc}, CSA\MPS{\ADDTEXT}{ }\cite{csa}
and CaSE\MPS{\ADDTEXT}{ }\cite{case,norton05alg}.  Our contribution in this paper is to
...
A number of calculi\MPS{\ADDTEXT}{ }\cite{lee:realtime,satoh:phd,webpi} have already
...
\item Encapsulation, realised in CaSE by the process of \emph{clock
      ...
      CSP\MPS{\ADDTEXT}{ }\cite{hoare:csp78}.  This facilities the amalgamation of clock ticks
...
area of \emph{localities}\MPS{\ADDTEXT}{ }\cite{obslocal,pawl} which have been used in
...
safe ambients\MPS{\ADDTEXT}{ }\cite{sangiorgi:mobsafeambients}.  \emph{Grave
